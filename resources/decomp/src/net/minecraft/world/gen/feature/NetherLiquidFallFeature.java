...
public class NetherLiquidFallFeature
extends Feature {
    private int liquid;

    public NetherLiquidFallFeature(int liquid) {
        this.liquid = liquid;
    }
    ...
    public boolean place(World world, Random random, int x, int y, int z) {
        ...
        if (world.getBlock(x, y, z) != 0 && world.getBlock(x, y, z) != Block.NETHERRACK.id) {
            return false;
        }
        int i = 0;
        if (world.getBlock(x - 1, y, z) == Block.NETHERRACK.id) {
            ++i;
        }
        if (world.getBlock(x + 1, y, z) == Block.NETHERRACK.id) {
            ++i;
        }
        if (world.getBlock(x, y, z - 1) == Block.NETHERRACK.id) {
            ++i;
        }
        if (world.getBlock(x, y, z + 1) == Block.NETHERRACK.id) {
            ++i;
