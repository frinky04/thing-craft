...
extends Generator {
                    ...
                    for (ad = q; !ac && ad < r; ++ad) {
                        for (int ae = w; !ac && ae < aa; ++ae) {
                            for (int ag = u + 1; !ac && ag >= t - 1; --ag) {
                                ...
                                if (blocks[ah] == Block.FLOWING_WATER.id || blocks[ah] == Block.WATER.id) {
                                    ac = true;
                                }
                    ...
                    if (!ac) {
                        for (ad = q; ad < r; ++ad) {
                            ...
                            for (int ai = w; ai < aa; ++ai) {
                                ...
                                boolean al = false;
                                for (int am = u - 1; am >= t; --am) {
                                    double an = ((double)am + 0.5 - y) / m;
                                    if (an > -0.7 && af * af + an * an + aj * aj < 1.0) {
                                        byte ao = blocks[ak];
                                        if (ao == Block.GRASS.id) {
                                            al = true;
                                        }
                                        if (ao == Block.STONE.id || ao == Block.DIRT.id || ao == Block.GRASS.id) {
                                            if (am < 10) {
                                                blocks[ak] = (byte)Block.FLOWING_LAVA.id;
                                            } else {
                                                blocks[ak] = 0;
                                                if (al && blocks[ak - 1] == Block.DIRT.id) {
                                                    blocks[ak - 1] = (byte)Block.GRASS.id;
                                                }
                                            }
                                        }
                                    }
    ...
    protected void place(World world, int startChunkX, int startChunkZ, int chunkX, int chunkZ, byte[] blocks) {
        int i = this.random.nextInt(this.random.nextInt(this.random.nextInt(40) + 1) + 1);
        if (this.random.nextInt(15) != 0) {
            i = 0;
        }
        for (int j = 0; j < i; ++j) {
            double d = startChunkX * 16 + this.random.nextInt(16);
            double e = this.random.nextInt(this.random.nextInt(120) + 8);
            ...
            for (int l = 0; l < k; ++l) {
                ...
                this.carveTunnel(chunkX, chunkZ, blocks, d, e, f, m, g, h, 0, 0, 1.0);
            }
        }
    }
}
