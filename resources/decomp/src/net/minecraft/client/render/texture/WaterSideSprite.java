...
extends DynamicTexture {
    ...
    public WaterSideSprite() {
        super(Block.FLOWING_WATER.sprite + 1);
        this.replicate = 2;
    }
    ...
    public void tick() {
        ...
        for (i = 0; i < 16; ++i) {
            for (int j = 0; j < 16; ++j) {
                ...
                this.next[i + j * 16] = f / 3.2f + this.heat[i + j * 16] * 0.8f;
            }
        }
        for (i = 0; i < 16; ++i) {
            for (int k = 0; k < 16; ++k) {
                ...
                int n2 = i + k * 16;
                this.heatDelta[n2] = this.heatDelta[n2] - 0.3f;
        ...
        this.current = fs;
        for (int l = 0; l < 256; ++l) {
            float g = this.current[l - this.ticks * 16 & 0xFF];
            if (g > 1.0f) {
                g = 1.0f;
            }
            if (g < 0.0f) {
