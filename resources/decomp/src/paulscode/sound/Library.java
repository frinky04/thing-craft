...
public class Library {
    ...
    public void init() {
        ...
        int i;
        Object object = null;
        for (i = 0; i < SoundSystemConfig.getNumberStreamingChannels() && (channel = this.createChannel(1)) != null; ++i) {
            this.streamingChannels.add(channel);
        }
        for (i = 0; i < SoundSystemConfig.getNumberNormalChannels() && (channel = this.createChannel(0)) != null; ++i) {
    ...
    public int feedRawAudioData(Source source, byte[] bs) {
        ...
        if (!source.playing() || source.channel == null) {
            ...
            if (source.channel != null && source.channel.attachedSource == source) {
                ...
                channel2 = this.getNextChannel(source);
            }
            int i = source.feedRawAudioData(channel2, bs);
            channel2.attachedSource = source;
    ...
    private Channel getNextChannel(Source source) {
        ...
        if (source.toStream) {
            int k = this.nextStreamingChannel;
            List list = this.streamingChannels;
            String[] stringArray = this.streamingChannelSourceNames;
        } else {
            l = this.nextNormalChannel;
            list2 = this.normalChannels;
            strings2 = this.normalChannelSourceNames;
        }
        int j = list2.size();
        for (i = 0; i < j; ++i) {
            if (!string.equals(strings2[i])) continue;
            return (Channel)list2.get(i);
        }
        int m = l;
        for (i = 0; i < j; ++i) {
            String string2 = strings2[m];
            if (string2 == null) {
                Object var10_10 = null;
            } else {
                source2 = (Source)this.sourceMap.get(string2);
            }
            if (source2 == null || !source2.playing()) {
                if (source.toStream) {
                    this.nextStreamingChannel = m + 1;
                    if (this.nextStreamingChannel >= j) {
                        this.nextStreamingChannel = 0;
                    }
                } else {
                    this.nextNormalChannel = m + 1;
                    if (this.nextNormalChannel >= j) {
                        this.nextNormalChannel = 0;
                    }
                }
                strings2[m] = string;
                return (Channel)list2.get(m);
            }
            if (++m < j) continue;
            m = 0;
        }
        ...
        for (i = 0; i < j; ++i) {
            Source source3;
            String string3 = strings2[m];
            if (string3 == null) {
                source2 = null;
            } else {
                source3 = (Source)this.sourceMap.get(string3);
            }
