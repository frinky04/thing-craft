...
- [x] Survival-loop HUD/vitals pass added: HUD renderer now uses Alpha GUI/icon textures with terrain-backed hotbar item sprites and heart rendering, player ECS includes explicit vitals/damage timers plus walk-bob state, fall-damage is applied via collision landing resolution, and camera view-bobbing now follows Alpha-style walk/bob/tilt transforms. Fall-distance accumulation now tracks net vertical drop across an airtime segment so jump ascent offsets small-ledge descent (matching current Alpha repro behavior).
...
- [x] Fluid locomotion follow-up added: player physics now includes Alpha-style water/lava movement integration (fluid drag, fluid gravity, swim-up impulse while jump is held, and horizontal-collision surface assist), with medium-contact state tracked on the player `PhysicsBody`. Surface assist now matches Alpha `canMove` liquid gating so submerged destinations no longer trigger underwater auto-hop.
...
- [x] App orchestration refactor started: benchmark mode config/runtime and screenshot/output helpers were extracted from `app.rs` into `app/bench.rs`, reducing event-loop module surface area while keeping behavior unchanged.
...
- [x] Block-interaction architecture follow-up added: right-click block interactions now use a place-first/use-second decision path with explicit menu actions (`CraftingTable`, `Chest`, `Furnace`) with typed menu context payloads (block positions / double-chest pair ordering).
- [x] Container/smelting runtime follow-up added: `ContainerRuntimeState` ECS resource now owns chest/furnace inventories, chest + double-chest and furnace menus render from Alpha GUI assets (`container.png`, `furnace.png`), chest obstruction/open rules match Alpha behavior, chest contents spill on chest break (Alpha-style split-stack behavior), furnace fixed-tick smelting/fuel simulation with lit/unlit block swaps is integrated with world block metadata preservation, and furnace removal follows Alpha block-entity removal semantics.
