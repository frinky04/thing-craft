...
- [~] Core Blocks: Stone, Cobblestone, Dirt, Grass, Wood, Planks, Leaves, Glass, Sand, Gravel, Bedrock. Oak, birch, and pine tree shapes now generate with log+leaf blocks (Alpha 1.2.6 uses a single log/leaf block type for all variants).
- [ ] Mechanics Blocks: TNT, Doors, Ladders, Torches, Signs, Furnaces, Crafting Tables, Chests.
- [~] Liquids: Water renders as semi-transparent blue with correct atlas textures; lava renders opaque with correct textures. Flow simulation (spreading, source-to-flowing conversion) and lava-water interaction remain pending.
