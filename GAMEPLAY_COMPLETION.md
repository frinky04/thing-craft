...
- [x] Mining Loop: Left-click mining now follows Alpha-style hold-to-break flow (`startMiningBlock`/`tickBlockMining`/`stopMiningBlock`) in fixed tick, including target retarget resets, per-tick progress accumulation from `playerSpeed / hardness / 30`, water/airborne penalties (`/5` each), zero-hardness instant break behavior, and 5-tick post-break cooldown. Tool-assisted mining entry points are stubbed for future tool/tier integration.
