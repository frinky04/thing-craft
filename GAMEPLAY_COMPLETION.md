...
- [~] Inventory Model: Runtime now uses an Alpha-style player inventory domain (27 main + 9 hotbar + 4 armor stubs) with cursor stack state, authoritative command-applied click semantics (left/right split/merge/swap, outside drop), interactive inventory panel (`E`) from `inventory.png`, integrated player 2x2 crafting, and crafting-table 3x3 crafting menu support using Alpha `CraftingManager` shaped recipe parity (mirroring + size-priority + result-slot ingredient consumption). Chest/furnace inventory models remain stubbed.
...
- [~] Mechanics Blocks: TNT, Doors, Ladders, Torches, Signs, Furnaces, Crafting Tables, Chests. Crafting table interaction/menu is now wired (3x3 crafting). Chest/furnace right-click menu stubs are in place; full container/smelting behavior is still pending.
