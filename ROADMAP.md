...
- M7 movement parity correction landed: fluid wall-surface jump assist now matches Alpha `canMove` liquid gating (assist is blocked when the destination AABB is still in water/lava, preventing underwater auto-hop against submerged blocks), and fall-distance accumulation now uses net vertical drop over an airtime segment so jump ascent offsets small-ledge descent (matching current Alpha repro behavior for 2-block pillar jump-offs).
...
- M7 crafting/menu follow-up landed: world right-click block interaction now runs through a place-first/use-second action pipeline (future-proof for block menus), with `CraftingTable` opening a `3x3` crafting menu wired to the same ECS `CraftingRegistry` as player `2x2` crafting.
- M7 container/smelting follow-up landed: chest + double-chest inventories and furnace inventories are now runtime-backed ECS resources, right-click chest/furnace opens Alpha-style dedicated menus (`container.png` / `furnace.png`) with slot-correct interaction routing, chest-open obstruction checks, container break-item spill, fixed-tick furnace smelting/fuel progression (Alpha mappings/timers), and lit/unlit furnace block-state toggling while preserving metadata facing. Dungeon chest/spawner payload generation remains deferred.
...
- Runtime architecture follow-up in progress: `app.rs` orchestration slimming has started by extracting benchmark-mode runtime/config/output code into `app/bench.rs` (behavior-preserving refactor; more subsystem extraction pending).
...
- Worldgen parity follow-up: decoration predicates were tightened toward Alpha semantics (flowers/mushrooms/pumpkins/cactus), snow placement now keys off `blocksMovement`, lake boundary checks now use registry material traits with corrected shoreline grass conversion, a generation-time sand/gravel settling pass approximates `FallingBlock.fallImmediately`, and populate now runs through a unified per-source-chunk RNG stream (Alpha ordering across all decorators). Spring immediate liquid tick now has a generation-time one-tick approximation; full runtime-equivalent behavior remains pending streaming/runtime hook-up.
- Worldgen tree parity follow-up landed during M7: chunk tree density now uses Alpha `forestNoise` weighting + biome offsets (including no Swampland bonus), tree feature selection is per source chunk (not per-tree), runtime Birch/Pine placement has been removed from default population, and `LargeOakTreeFeature` branching/trunk/foliage placement is now ported for default decoration.
