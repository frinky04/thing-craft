...
- M7 inventory-system modernization landed (crafting deferred): hotbar-only state has been replaced by a command-applied player inventory domain (`27 main + 9 hotbar + 4 armor stubs + cursor stack`) with authoritative fixed-tick transaction processing, Alpha-style left/right click slot semantics, outside-drop behavior, and an interactive `E` inventory screen rendered from `gui/inventory.png`.
