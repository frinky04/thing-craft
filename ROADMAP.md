...
- Worldgen parity follow-up: decoration predicates were tightened toward Alpha semantics (flowers/mushrooms/pumpkins/cactus), snow placement now keys off `blocksMovement`, lake boundary checks now use registry material traits with corrected shoreline grass conversion, and a generation-time sand/gravel settling pass now approximates `FallingBlock.fallImmediately` during populate. Spring immediate liquid tick parity is still pending streaming/runtime hook-up.
